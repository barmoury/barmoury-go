...
func (m *JSONMap[T]) Scan(val interface{}) error {
	if val == nil {
		*m = make(JSONMap[T])
		return nil
	}
...
func (m *JSONMap[T]) UnmarshalJSON(b []byte) error {
	t := map[string]T{}
...
func (m JSONMap[T]) GormDataType() string {
	return "jsonmap"
}
...
func (jm JSONMap[T]) GormValue(ctx context.Context, db *gorm.DB) clause.Expr {
